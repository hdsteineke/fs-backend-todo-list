...
class Todo {
  ...
  static async insert({ user_id, task, details, created_at }) {
    const { rows } = await pool.query(
      `
      INSERT INTO todos (user_id, task, details, created_at)
      VALUES ($1, $2, $3, $4)
      RETURNING *`,
      [user_id, task, details, created_at]
    );
    return new Todo(rows[0]);
  }
